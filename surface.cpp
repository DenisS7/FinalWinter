...
namespace GameSpace
{
    ...
    Surface::Surface(int a_Width, int a_Height) :
        ...
        m_Flags(OWNER)
    {
        m_Buffer = static_cast<Pixel*>(MALLOC64(
            static_cast<unsigned>(a_Width) * static_cast<unsigned>(a_Height) * sizeof( Pixel )));
    }
    ...
    Surface::Surface(char* a_File, int a_Height, int a_Width)
    {
        ...
        m_Buffer = static_cast<Pixel*>(MALLOC64(m_Width * m_Height * sizeof(Pixel)));
        if (m_Buffer)
        {
            m_Flags = OWNER;
            assert(m_Pitch != 0);
    ...
    {
        int x = (m_Width - static_cast<int>(strlen(a_String)) * 6) / 2;
    ...
    void Surface::Print(char* a_String, int x1, int y1, Pixel color)
    {
        ...
        for (int i = 0; i < static_cast<int>(strlen(a_String)); i++, t += 6)
        {
            long pos = 0;
            if ((a_String[i] >= 'A') && (a_String[i] <= 'Z')) pos = s_Transl[static_cast<unsigned short>(a_String[i] - (
                'A' - 'a'))];
            else pos = s_Transl[static_cast<unsigned short>(a_String[i])];
            Pixel* a = t;
            auto c = (char*)s_Font[pos];
    ...
    void Surface::Resize(Surface* a_Orig)
    {
        ...
        for (v = 0; v < m_Height; v++)
        {
            for (u = 0; u < m_Width; u++)
            {
                ...
                unsigned int r = (((p1 & RedMask) * w1 + (p2 & RedMask) * w2 + (p3 & RedMask) * w3 + (p4 & RedMask) *
                    w4) >> 8) & RedMask;
                unsigned int g = (((p1 & GreenMask) * w1 + (p2 & GreenMask) * w2 + (p3 & GreenMask) * w3 + (p4 &
                    GreenMask) * w4) >> 8) & GreenMask;
                unsigned int b = (((p1 & BlueMask) * w1 + (p2 & BlueMask) * w2 + (p3 & BlueMask) * w3 + (p4 & BlueMask)
                    * w4) >> 8) & BlueMask;
                *(dst + u + v * m_Pitch) = r + g + b;
            }
        }
    }
    ...
    void Surface::Line(float x1, float y1, float x2, float y2, Pixel c)
    {
        ...
        for (int i = 0; i <= il; i++)
        {
            *(m_Buffer + static_cast<int>(x1) + static_cast<int>(y1) * m_Pitch) = c;
            x1 += dx, y1 += dy;
        }
    }
    ...
    void Sprite::DrawScaled(int a_X, int a_Y, int a_Width, int a_Height, Surface* a_Target)
    {
        ...
        for (int x = 0; x < a_Width; x++)
            ...
            {
                int u = static_cast<int>(static_cast<float>(x) * (static_cast<float>(m_Width) / static_cast<float>(
                    a_Width)));
                int v = static_cast<int>(static_cast<float>(y) * (static_cast<float>(m_Height) / static_cast<float>(
                    a_Height)));
    ...
    Font::Font(char* a_File, char* a_Chars)
    {
        ...
        unsigned int i;
        for (i = 0; i < strlen(a_Chars); i++) m_Trans[static_cast<unsigned char>(a_Chars[i])] = i;
    ...
    int Font::Width(char* a_Text)
    {
        ...
        {
            unsigned char c = static_cast<unsigned char>(a_Text[i]);
            if (c == 32) w += 4;
    ...
    void Font::Print(Surface* a_Target, char* a_Text, int a_X, int a_Y, bool clip)
    {
        ...
        for (cx = 0, i = 0; i < strlen(a_Text); i++)
        {
            ...
            {
                int c = m_Trans[static_cast<unsigned char>(a_Text[i])];
                    ...
                    for (y = 0; y < m_Height; y++)
                    {
                        if (((a_Y + y) >= m_CY1) && ((a_Y + y) <= m_CY2))
                        {
                            for (x = 0; x < m_Width[c]; x++)
                                if ((t[x]) && ((x + static_cast<int>(cx) + a_X) < a_Target->GetPitch()))
                ...
                cx += m_Width[c] + 2;
                if (static_cast<int>(cx + a_X) >= a_Target->GetPitch()) break;
            }
        }
    }
}; // namespace Tmpl8
