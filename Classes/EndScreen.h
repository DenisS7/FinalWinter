...
namespace UI
{
    ...
    public:
            ...
            ScreenBase(screen)
        {
            ...
            GameSpace::vec2 pos(static_cast<float>(screen->GetPitch()) / 2 - buttons[0]->getSize().x / 2,
                                static_cast<float>(screen->GetHeight()) / 2 - buttons[0]->getSize().y);
            buttons[0]->setDrawLocation(pos);
            pos = GameSpace::vec2(static_cast<float>(screen->GetPitch()) / 2 - buttons[1]->getSize().x / 2,
                                  static_cast<float>(screen->GetHeight()) / 2 + static_cast<float>(screen->GetHeight())
                                  / 5 - buttons[1]->getSize().y);
            buttons[1]->setDrawLocation(pos);
            pos = GameSpace::vec2(static_cast<float>(screen->GetPitch()) / 2 - buttons[1]->getSize().x / 2,
                                  static_cast<float>(screen->GetHeight()) / 2 + static_cast<float>(2) * screen->
                                  GetHeight() / 5 - buttons[2]->getSize().y);
