...
namespace UI
{
    ...
    public:
            ...
            ScreenBase(screen)
        {
            ...
            screenType = 2;
            GameSpace::vec2 pos(static_cast<float>(screen->GetPitch()) / 2 - buttons[1]->getSize().x / 2,
                                3 * static_cast<float>(screen->GetHeight()) / 5);
            buttons[1]->setDrawLocation(pos);

            pos = GameSpace::vec2(static_cast<float>(screen->GetPitch()) / 2 - buttons[1]->getSize().x / 2,
                                  4 * static_cast<float>(screen->GetHeight()) / 5);
