...
namespace Character
{
    ...
    {
        int x = (static_cast<int>(locf.x) + posToGet.x) / currentRoom->tilesize;
        int y = (static_cast<int>(locf.y) + posToGet.y) / currentRoom->tilesize;
    ...
    void EnemyBase::init(int type)
    {
        ...
        locf.x = static_cast<float>(IRand((currentRoom->getSize().x - sprite->GetWidth() / currentRoom->tilesize - 4) * currentRoom->tilesize)) + currentRoom->tilesize * 2;
        locf.y = static_cast<float>(IRand((currentRoom->getSize().y - sprite->GetHeight() / currentRoom->tilesize - 4) * currentRoom->tilesize)) + currentRoom->tilesize * 2;

        data.col.collisionBox.x = data.col.offset.x + static_cast<int>(locf.x);
        data.col.collisionBox.y = data.col.offset.y + static_cast<int>(locf.y);
        ...
        //modify the tiles that this enemy is part of
    ...
    void EnemyBase::takeDamage(const int damage)
    {
        ...
        if (!isFollowingPlayer && !isDead) //start following player when the enemy is hit by it
            triggerFollowPlayer();
    }
