    ...
    public:
        IceExplosion(int x, int y, int damageBoost, Character::Player* newPlayer)
        {
            ...
            *sprite = *Sprites::get().attack[0];
            damage += damageBoost;
            drawLocf.x = static_cast<float>(x) - sprite->GetWidth() / 2;
            drawLocf.y = static_cast<float>(y) - sprite->GetHeight() / 2;
            col.setOffset(3, 3);
            col.setCollisionBox(x + col.offset.x, y + col.offset.y, 57, 57);
